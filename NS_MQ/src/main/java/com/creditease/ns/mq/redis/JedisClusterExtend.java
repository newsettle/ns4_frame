...
public class JedisClusterExtend implements JedisCommands, BasicCommands, Closeable {
	...
	public JedisClusterExtend(Set<HostAndPort> nodes, final GenericObjectPoolConfig poolConfig) {
		this(nodes, DEFAULT_TIMEOUT, DEFAULT_MAX_REDIRECTIONS, poolConfig, null);
	}

	public JedisClusterExtend(Set<HostAndPort> nodes, int timeout, final GenericObjectPoolConfig poolConfig) {
		this(nodes, timeout, DEFAULT_MAX_REDIRECTIONS, poolConfig, null);
	}

	public JedisClusterExtend(Set<HostAndPort> jedisClusterNode, int timeout, int maxRedirections,
			final GenericObjectPoolConfig poolConfig, final String password) {
		this(jedisClusterNode, timeout, timeout, maxRedirections, poolConfig, password);
	}

	public JedisClusterExtend(Set<HostAndPort> jedisClusterNode, int connectionTimeout, int soTimeout,
    ...
    @Deprecated
    public Object eval(final byte[] script, final byte[] key, final int keyCount, final byte[]... params) {
        return new JedisClusterCommand<Object>(connectionHandler, maxRedirections) {
            @Override
            public Object execute(Jedis connection) {
                return connection.eval(script, toByteArray(keyCount), params);
            }
        }.run(SafeEncoder.encode(key));
    }


    public Long pushAndExpirte(final int second,final byte[] key, final byte[]... string) {
        return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
            @Override
            public Long execute(Jedis connection) {
                long result = -1;
                Long status = connection.lpush(key, string);
                result++;
                if (status >= 0)
                {
                    try {
